...
nn_loss2 = np.zeros((num_steps-1)/50+1)

batch_size = 128
n_hidden_nodes_12 = 300
n_hidden_nodes_23 = 200
beta = 0.2
alpha = 0.1
...
with graph.as_default():
    ...
    loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(z_23, tf_y_train))
    #loss with regularization
    loss = loss + beta * (tf.nn.l2_loss(weights_01) + tf.nn.l2_loss(weights_12) + tf.nn.l2_loss(weights_23) )
    ...
    valid_prediction = tf.nn.softmax(
        tf.matmul(tf.nn.relu(tf.matmul(tf.nn.relu(tf.matmul(tf_X_valid, weights_01) + biases_01), 
                                       weights_12) + biases_12), weights_23)+biases_23)
    test_prediction = tf.nn.softmax(
        tf.matmul(tf.nn.relu(tf.matmul(tf.nn.relu(tf.matmul(tf_X_test, weights_01) + biases_01), 
                                       weights_12) + biases_12), weights_23)+biases_23)
...
h5f = h5py.File('nn_3_data.h5', 'w')
h5f.create_dataset('nn_loss2', data=nn_loss2)
h5f.close()
