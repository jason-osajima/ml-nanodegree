...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        if step % display_step == 0:
            ...
            print "Iter " + str(step*batch_size) + ", Minibatch Loss= " + \
                  ...
                  "{:.5f}".format(acc)
            cnn_2_step= np.append(cnn_2_step, step*batch_size)
            cnn_2_loss= np.append(cnn_2_loss, loss)
    ...
    cnn_2_cm = sklearn.metrics.confusion_matrix(y_true, y_pred)
...
h5f = h5py.File('cnn_2_data.h5', 'w')
h5f.create_dataset('cnn_2_loss', data=cnn_2_loss)
h5f.create_dataset('cnn_2_step', data=cnn_2_step)
h5f.create_dataset('cnn_2_cm', data=cnn_2_cm)
h5f.close()